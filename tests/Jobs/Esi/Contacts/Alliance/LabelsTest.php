...
class LabelsTest extends JobEsiTestCase
{
    public static function setUpBeforeClass(): void
    {
        ...
        $response_success = new EsiResponse(
            file_get_contents(__DIR__ . '/../../../../artifacts/contacts/alliances/labels.json'),
            [
    ...
    public function testHandleSuccess()
    {
        ...
        $job = new Labels(99000001, $token);
        $job->handle();

        $labels = AllianceLabel::all();

        $data = json_encode(ContactLabelResource::collection($labels));
        $this->assertJsonStringEqualsJsonFile(__DIR__ . '/../../../../artifacts/contacts/alliances/labels.json', $data);
    }
    ...
    public function testHandleNotModified()
    {
        ...
        AllianceLabel::create([
            'alliance_id' => 99000001,
            'label_id'    => 1,
            'name'        => 'Alliance Enemies',
        ]);

        AllianceLabel::create([
            'alliance_id' => 99000001,
            'label_id'    => 2,
            'name'        => 'Alliance Allies',
        ]);

        AllianceLabel::create([
            'alliance_id' => 99000001,
            'label_id'    => 3,
            'name'        => 'Alliance Friends',
        ]);
    ...
    public function testHandleUpdated()
    {
        ...
        EsiInMemoryCache::getInstance()->forget('/v1/alliances/99000001/contacts/labels/', 'datasource=tranquility');
        ...
        $labels = AllianceLabel::all();
        $data = json_encode(ContactLabelResource::collection($labels));
        $this->assertJsonStringNotEqualsJsonFile(__DIR__ . '/../../../../artifacts/contacts/alliances/labels.json', $data);
