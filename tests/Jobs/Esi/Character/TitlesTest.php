...
class TitlesTest extends JobEsiTestCase
{
    public static function setUpBeforeClass(): void
    {
        ...
        $response_success = new EsiResponse(
            file_get_contents(__DIR__ . '/../../../artifacts/characters/titles.json'),
            [
    ...
    public function testHandleSuccess()
    {
        ...
        $character->affiliation()->create([
            'corporation_id' => 109299958,
        ]);

        $job = new Titles($token);
        $job->handle();

        $titles = $character->titles;

        $data = json_encode(TitleResource::collection($titles));
        $this->assertJsonStringEqualsJsonFile(__DIR__ . '/../../../artifacts/characters/titles.json', $data);
    }
    ...
    public function testHandleNotModified()
    {
        ...
        $title = CorporationTitle::create([
            'corporation_id' => 109299958,
            'title_id'       => 1,
            'name'           => 'Awesome Title',
        ]);
        $character->titles()->attach($title);
        ...
        $titles = CharacterInfo::find(180548812)->titles;

        $this->assertEquals(2, $titles->count());
    }
    ...
    public function testHandleUpdated()
    {
        ...
        EsiInMemoryCache::getInstance()->forget('/v1/characters/180548812/titles/', 'datasource=tranquility');
        ...
        $titles = $character->titles;
        $data = json_encode(TitleResource::collection($titles));
        $this->assertJsonStringNotEqualsJsonFile(__DIR__ . '/../../../artifacts/characters/titles.json', $data);

        $job = new Titles($token);
        $job->handle();

        $character->load('titles');
