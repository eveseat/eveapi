...
use Seat\Eveapi\Models\Industry\CharacterJob;
    ...
    public function handle()
    {
        ...
        collect($industry_jobs)->each(function ($job) {
            ...
            CharacterJob::firstOrNew([
                ...
                'runs'                   => $job->runs,
                'cost'                   => property_exists($job, 'cost') ?
                    $job->cost : null,
                'licensed_runs'          => property_exists($job, 'licensed_runs') ?
                    $job->licensed_runs : null,
                'probability'            => property_exists($job, 'probability') ?
                    $job->probability : null,
                'product_type_id'        => property_exists($job, 'product_type_id') ?
                ...
                'completed_character_id' => property_exists($job, 'completed_character_id') ?
                    $job->completed_character_id : null,
                'successful_runs'        => property_exists($job, 'successful_runs') ?
